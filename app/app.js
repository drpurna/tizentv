// ================================================================
// IPTV — app.js v10.0 | Samsung TV / TizenBrew
// ================================================================
// v10 FIXES:
// • Render proxy: houser-3q3n.onrender.com (Singapore IP — Jio allows)
// • NEW JioTV API endpoints from JioTV-Go v3 source
// • Mobile number Base64 encoded (required by Jio v1 API)
// • sendOTP returns HTTP 204 No Content = success (handled)
// • verifyOTP payload includes deviceInfo object
// • Correct headers: appname=RJIL_JioTV
// ================================================================

const PROXY = 'https://houser-3q3n.onrender.com';

// ── JioTV API endpoints (v3 — verified from JioTV-Go source) ──
...
/* ================================================================
   PROXY XHR — all Jio API calls via Render Singapore server
   Render server has Singapore IP — Jio accepts Asian IPs
================================================================ */
function proxyXHR(method, targetUrl, headers, body, timeoutMs){
  return new Promise((resolve, reject)=>{
    const proxyUrl = PROXY + '/proxy?url=' + encodeURIComponent(targetUrl);
    ...
    xhr.onload = function(){
      // 204 No Content = sendOTP success
    ...
    xhr.onerror   = ()=> reject(new Error('Cannot reach proxy — check internet connection'));
    xhr.ontimeout = ()=> reject(new Error('Request timed out — proxy may be waking up, try again'));
...
function jioHdr(withAuth){
  ...
  if(withAuth && jioCred){
    ...
    // Legacy headers for older endpoints
    h['appKey']           = 'NzNiMDhlYzQyNjJm';
    h['channelPartnerID'] = 'JIOTV';
    h['cId']              = 'com.jio.media.jiotv';
    h['lId']              = '6';
    h['dm']               = '1';
  }
  return h;
...
/* ── JioTV API calls — v3 correct implementation ────────── */
function jioSendOTP(mobile){
  // Number MUST be Base64 encoded — Jio v1 API requirement
  const b64 = btoa(mobile);
  return proxyXHR('POST', JIO_SEND_OTP, jioHdr(false),
    { number: b64 }, 25000
  ).then(data=>{
    // HTTP 204 = success (resolve({}) above) — no OTPId returned
    // HTTP 200 with body = also possible
...
function jioVerifyOTP(mobile, otp){
  ...
  return proxyXHR('POST', JIO_VERIFY_OTP, jioHdr(false),
    ...
    return {
      ...
      uniqueId:          uid,
      crm:               sub,
      accessTokenExpiry: Date.now()+(60*60000),
    };
  });
}
...
function jioRefreshTokens(){
  ...
  return proxyXHR('POST', JIO_REFRESH_AT, jioHdr(true),
    {refreshToken:jioCred.refreshToken||''}, 15000
  ).then(data=>{
    const t=data.authToken||data.accessToken;
    if(!t) throw new Error('No token in refresh');
    jioCred.accessToken=t; jioCred.accessTokenExpiry=Date.now()+(60*60000); jioSaveCred();
  }).catch(()=>
    proxyXHR('POST', JIO_REFRESH_SSO, jioHdr(true), {}, 15000)
    .then(data=>{
      const t=data.ssoToken||data.authToken;
      if(!t) throw new Error('Session expired — please log in again');
      jioCred.ssoToken=t; jioCred.accessTokenExpiry=Date.now()+(90*60000); jioSaveCred();
    })
  );
}

function jioGetStreamUrl(channelId){
  const maybeRefresh=(jioCred.accessTokenExpiry&&jioCred.accessTokenExpiry-Date.now()<300000)
    ?jioRefreshTokens().catch(()=>{}):Promise.resolve();
  return maybeRefresh.then(()=>
    proxyXHR('POST', JIO_STREAM, jioHdr(true),
      {channel_id:String(channelId),liveurl:'1'}, 15000)
  ).then(data=>{
    const u=data.result||data.url||data.stream_url||data.streamUrl||'';
    if(!u) throw new Error(data.message||data.errorMessage||'No stream URL');
    return u;
  });
}
...
function loadJioChannels(){
  ...
  const maybeRefresh=(jioCred&&jioCred.accessTokenExpiry&&jioCred.accessTokenExpiry-Date.now()<300000)
    ?jioRefreshTokens().catch(()=>{}):Promise.resolve();
  return maybeRefresh.then(()=>
    proxyXHR('GET', JIO_CHANNELS, jioHdr(true), null, 30000)
  ).then(data=>{
...
function jioLoadCred(){
  try{ const raw=localStorage.getItem(JIO_CRED_KEY); if(!raw) return null; const c=JSON.parse(raw); if(!c||(!c.ssoToken&&!c.accessToken)) return null; return c; }catch(e){ return null; }
}
function handleJioTab(){
  ...
  if(jioCred){ loadJioChannels().then(ok=>{ if(!ok){ jioClearCred(); showJioLoginModal(); } }); return; }
  showJioLoginModal();
}
function initJio(){
  try{ const c=jioLoadCred(); if(!c) return; jioCred=c; if(plIdx===JIO_IDX) loadJioChannels(); }catch(e){ console.warn('[Jio] initJio:',e.message); }
}
...
const VS={
  ...
  setData(n){ this.total=n; this.rs=-1; this.re=-1; this.inner.textContent=''; this.nodes=[]; this.inner.style.cssText='position:relative;width:100%;height:'+(n*this.ITEM_H)+'px;'; this.c.scrollTop=0; this.st=0; this.vh=this.c.clientHeight||this.c.offsetHeight||700; this._paint(); },
  ...
  _paint(){
    ...
    for(const nd of this.nodes){ const on=nd._i===sel; if(on!==nd._on){ nd._on=on; nd.classList.toggle('active',on); if(nd._nm) nd._nm.style.color=on?'#000':''; if(nd._nu) nd._nu.style.color=on?'#999':''; } }
  },
  _build(i){
    ...
    const logo=ch.logo?`<div class="ch-logo"><img src="${esc(ch.logo)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextSibling.style.display='flex'"/><span class="ch-logo-fb" style="display:none">${ini}</span></div>`:`<div class="ch-logo"><span class="ch-logo-fb">${ini}</span></div>`;
...
function renderList(){
  ...
  if(!filtered.length){ VS.setData(0); const li=document.createElement('li'); li.style.cssText='position:absolute;top:0;left:0;right:0;padding:24px 16px;color:#444;'; li.textContent='No channels'; VS.inner.appendChild(li); return; }
...
function applyAutoAspect(){ if(arManuallySet) return; const W=video.videoWidth,H=video.videoHeight; if(!W||!H) return; const sAR=W/H,scAR=16/9; if(H<=576){ const wi=AR_MODES.findIndex(m=>m.cls==='ar-wide'); if(arIdx!==wi) setAR(wi,`Auto Wide (${H}p)`); }else if(Math.abs(sAR-scAR)<0.08){ const ci=AR_MODES.findIndex(m=>m.cls==='ar-cover'); if(arIdx!==ci) setAR(ci,`Auto Fill (${W}\u00d7${H})`); }else{ if(arIdx!==0) setAR(0,`Auto Fit (${W}\u00d7${H})`); } }
...
function startPreview(idx){
  ...
  if(ch.channelId){
    ...
    jioGetStreamUrl(ch.channelId)
      ...
      .catch(err=>{
        if(/401|token|auth/i.test(err.message)){
          jioRefreshTokens()
...
async function doPlayPreview(ch,idx){
  ...
  video.addEventListener('playing',function onPl(){ video.removeEventListener('playing',onPl); applyAutoAspect(); });
  ...
  try{
    ...
    shakaPlayer.addEventListener('error',e=>{ const err=e.detail; if(err.severity===shaka.util.Error.Severity.CRITICAL){ setStatus('Error '+err.code,'error'); finishLoadBar(); }else{ setStatus('Buffering\u2026','loading'); } });
...
function handleModalKey(e){
  ...
  if(k==='Escape'||k==='Back'||k==='GoBack'||c===10009||c===27){ if(!step1Vis){ document.getElementById('jioLoginStep2').style.display='none'; document.getElementById('jioLoginStep1').style.display='block'; document.getElementById('jioOtp').value=''; }else{ document.getElementById('jioCloseModal').click(); } e.preventDefault(); e.stopImmediatePropagation(); return; }
...
function showJioLoginModal(){
  ...
  reqBtn.onclick=function(){
    ...
    st('Sending OTP\u2026 (may take 10-15s first time)','wait');
    ...
    jioSendOTP(mobile)
      ...
      .catch(err=>{ console.error('[Jio] sendOTP:',err); st('Error: '+err.message,'error'); reqBtn.disabled=false; });
  };
  ...
  verBtn.onclick=function(){
    ...
    jioVerifyOTP(pendingMobile, otp)
      .then(cred=>{ jioCred=cred; jioSaveCred(); st('\u2713 Logged in! Loading channels\u2026','ok'); setTimeout(()=>{ jioModalOpen=false; modal.style.display='none'; loadJioChannels().then(ok=>{ if(!ok) showToast('Channels failed — tap Jio TV again'); }); },600); })
      .catch(err=>{ console.error('[Jio] verifyOTP:',err); st('Error: '+err.message,'error'); verBtn.disabled=false; });
  };

  if(resndBtn){ resndBtn.onclick=function(){ step2.style.display='none'; step1.style.display='block'; otpIn.value=''; reqBtn.disabled=false; st('Re-enter number to resend OTP','wait'); }; }
